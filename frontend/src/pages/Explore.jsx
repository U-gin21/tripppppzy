      ...
      <div className="card glass-card p-4 border-0 mb-5">
        <div className="row g-3">
          ...
          <div className="col-md-3">
            ...
            <select className="form-select rounded-3" value={district} onChange={(e) => setDistrict(e.target.value)}>
          ...
          <div className="col-md-3">
            ...
            <select className="form-select rounded-3" value={interest} onChange={(e) => setInterest(e.target.value)}>
          ...
          <div className="col-md-3">
            ...
            <select className="form-select rounded-3" value={budget} onChange={(e) => setBudget(e.target.value)}>
            ...
            <div className="col-md-6 col-lg-4" key={dest.id}>
              ...
              >
                <div style={{ height: '220px', overflow: 'hidden', position: 'relative' }}>
                  ...
                  />
                  <span className="badge bg-success bg-opacity-95 position-absolute top-0 end-0 m-3 px-3 py-2 rounded-pill">
                    {dest.interest_category}
                  </span>
                </div>
                <div className="card-body p-4">
                  <span className="text-primary small fw-bold uppercase"><i className="bi bi-geo-alt-fill"></i> {dest.district} District</span>
                  <h4 className="fw-bold mt-1 text-gradient">{dest.name}</h4>
                  <p className="text-muted small line-clamp-3">{dest.description.substring(0, 120)}...</p>
                  <div className="d-flex justify-content-between align-items-center mt-3 pt-3 border-top">
                    <span className="badge bg-secondary bg-opacity-10 text-dark text-capitalize">{dest.budget_category}</span>
                    <button className="btn btn-outline-gradient btn-sm rounded-pill">View Details & Weather</button>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="modal-body p-4">
                  <div className="row g-4">
                    <div className="col-md-6">
                      <img 
                        src="https://images.unsplash.com/photo-1588598130836-8e562c161ab8?auto=format&fit=crop&w=800&q=80" 
                        alt={selectedDest.name} 
                        className="w-100 rounded-3 object-fit-cover"
                        style={{ maxHeight: '250px', objectFit: 'cover' }}
                      />
                      <div className="mt-3">
                        <span className="badge bg-primary bg-opacity-10 text-primary me-2 text-capitalize">{selectedDest.budget_category}</span>
                        <span className="badge bg-success bg-opacity-10 text-success">{selectedDest.interest_category}</span>
                      </div>
                      <h5 className="fw-bold mt-4">Key Activities</h5>
                      <p className="text-muted small">{selectedDest.activities}</p>
                      <h5 className="fw-bold mt-3">About</h5>
                      <p className="text-muted small">{selectedDest.description}</p>
                    </div>

                    <div className="col-md-6">
                      <div className="card border-0 bg-light p-4 rounded-3 h-100">
                        <h4 className="fw-bold mb-3"><i className="bi bi-cloud-sun text-info"></i> Climate & Weather Forecast</h4>
                        ...
                        {weatherData && !weatherLoading && (
                          <div>
                            <div className="weather-widget mb-3 text-center">
                              <h2 className="display-4 fw-bold mb-0">{weatherData.current.temperature_2m}°C</h2>
                              <p className="mb-0 small">Relative Humidity: {weatherData.current.relative_humidity_2m}%</p>
                              <p className="mb-0 small">Rain condition: {weatherData.current.rain > 0 ? "🌧️ Raining" : "☀️ Clear skies"}</p>
                            </div>

                            <div className="card bg-success bg-opacity-10 text-success border-0 p-3 rounded-3 mb-3 small">
                              <p className="mb-0 fw-bold">{getWeatherRecommendation(weatherData.current.rain, weatherData.daily)}</p>
                            </div>

                            <h6 className="fw-bold mb-2">7-Day Prediction</h6>
                                ...
                                return (
                                  <div className="col-3 text-center" key={day}>
                                    <div className="bg-white border rounded p-2 small">
                                      <span className="d-block fw-bold text-muted" style={{ fontSize: '10px' }}>{dayName}</span>
                                      <span className="d-block fw-bold">{Math.round(weatherData.daily.temperature_2m_max[idx])}°</span>
                                      <span className="text-muted" style={{ fontSize: '10px' }}>{Math.round(weatherData.daily.temperature_2m_min[idx])}°</span>
                                      <span className="d-block text-info mt-1" style={{ fontSize: '10px' }}>
